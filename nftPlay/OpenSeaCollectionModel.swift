...
//
//   let openSeaCollection = try? newJSONDecoder().decode(OpenSeaCollection.self, from: jsonData)

// Answer to https://stackoverflow.com/questions/48314724

import Foundation

// MARK: - OpenSeaCollection
struct OpenSeaCollection: Codable {
    let collections: [Collection]
}

// MARK: - Collection
struct Collection: Codable, Identifiable {   //TBD
    var id = UUID()
...
struct PrimaryAssetContract: Codable {
    ...
    let primaryAssetContractDescription, externalLink, imageURL: JSONNull?
    let defaultToFiat: Bool
